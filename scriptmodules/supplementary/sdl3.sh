...
function get_pkg_ver_sdl3() {
    local ver="$(get_ver_sdl3)"
    if [[ "$__os_debian_ver" -ge 11 ]]; then
        ver+="+1"
    else
        ver+="+5"
...
function get_arch_sdl3() {
    echo "$(dpkg --print-architecture)"
}

function _list_depends_sdl3() {
...
function depends_sdl3() {
    ...
    getDepends $(_list_depends_sdl3) "${depends[@]}"
}

function sources_sdl3() {
    local ver="$(get_ver_sdl3)"
    local pkg_ver="$(get_pkg_ver_sdl3)"
    local branch="retropie-${ver}"

    gitPullOrClone "$md_build/$pkg_ver" https://github.com/libsdl-org/SDL.git
    cd "$pkg_ver"
...
function build_sdl3() {
    local conf_flags=()
    local conf_depends=( $(_list_depends_sdl3) )
    ...
    md_ret_require="$md_build/libsdl3-dev_$(get_pkg_ver_sdl3)_$(get_arch_sdl3).deb"
...
function remove_old_sdl3() {
    # remove our old libsdl2 packages
    hasPackage libsdl3 && dpkg --remove libsdl3 libsdl3-dev
}
...
function install_sdl3() {
    ...
    if ! dpkg -i libsdl3-2.0-0_$(get_pkg_ver_sdl3)_$(get_arch_sdl3).deb libsdl3-dev_$(get_pkg_ver_sdl3)_$(get_arch_sdl3).deb; then
        apt-get -y -f --no-install-recommends install
    fi
    echo "libsdl3-dev hold" | dpkg --set-selections
}

function __binary_url_sdl3() {
    rp_hasBinaries && echo "$__binary_url/libsdl3-dev_$(get_pkg_ver_sdl3)_$(get_arch_sdl3).deb"
}

function install_bin_sdl3() {
    local tmp="$(mktemp -d)"
    pushd "$tmp" >/dev/null
    local ret=1
    if downloadAndVerify "$__binary_url/libsdl3-dev_$(get_pkg_ver_sdl3)_$(get_arch_sdl3).deb" && \
       downloadAndVerify "$__binary_url/libsdl3-2.0-0_$(get_pkg_ver_sdl3)_$(get_arch_sdl3).deb"; then
        install_sdl3
        ret=0
    fi
    popd >/dev/null
...
function revert_sdl3() {
    aptUpdate
    local packaged="$(apt-cache madison libsdl3-dev | cut -d" " -f3 | head -n1)"
    if ! aptInstall --allow-downgrades --allow-change-held-packages libsdl3-2.0-0="$packaged" libsdl3-dev="$packaged"; then
        md_ret_errors+=("Failed to revert to OS packaged sdl3 versions")
    fi
}

function remove_sdl3() {
    apt-get remove -y --allow-change-held-packages libsdl3-dev libsdl3-2.0-0
